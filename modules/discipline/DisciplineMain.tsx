          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-gray-50">
              {isLoading ? <tr><td colSpan={6} className="py-20 text-center text-gray-400">Memuat...</td></tr> : filteredWarnings.map(w => {
                ...
                return (
                  <tr key={w.id} className={`hover:bg-gray-50/50 transition-colors ${isSelected ? 'bg-emerald-50/20' : ''}`}>
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-gray-50">
              {isLoading ? <tr><td colSpan={7} className="py-20 text-center text-gray-400">Memuat...</td></tr> : filteredTerminations.map(t => {
                ...
                return (
                  <tr key={t.id} className={`hover:bg-gray-50/50 transition-colors border-l-2 border-l-red-500 ${isSelected ? 'bg-red-50/20' : 'bg-red-50/5'}`}>
                    <td className="px-6 py-4">
                      <input 
                        type="checkbox" 
                        className="rounded border-gray-300 text-[#006E62] focus:ring-[#006E62]"
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        {t.file_id && <a href={googleDriveService.getFileUrl(t.file_id)} target="_blank" className="p-1.5 text-[#006E62] hover:bg-emerald-50 rounded transition-colors"><Paperclip size={14} /></a>}
                        <button onClick={() => handleDeleteTermination(t.id, t.account_id)} className="p-1.5 text-gray-300 hover:text-red-500 transition-colors"><Trash2 size={14} /></button>
                      </div>
                    </td>
                  </tr>
                );
